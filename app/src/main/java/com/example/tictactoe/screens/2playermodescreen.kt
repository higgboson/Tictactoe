...
@Composable
fun TicTacToeGame(size: Int, playerName: String,onBack: () -> Unit) {
    val board = remember {
    ...
    )
    {
        ...
        BoxWithConstraints( // we use box with constraints to get a dynamic grid inside the screen , so that entire grid stays inside the screen even when we change the box size
            modifier = Modifier
                .fillMaxSize()
                .padding(16.dp)
        ) {
            ...
            Column(modifier = Modifier.fillMaxSize(),verticalArrangement = Arrangement.Center,horizontalAlignment = Alignment.CenterHorizontally)
            {
                        ...
                        for (j in 0 until size)
                        {
                            Box(
                                modifier = Modifier
                                    .size(boxSize)
                                    .padding(2.dp)
                                    .background(brush = Brush.linearGradient(colors = listOf(Color(0xFFFBAB7E), Color(0xFFF7CE68)) ), RoundedCornerShape(8.dp))
                                    .clickable
                                    {
                ...
                winner?.let {
                    ...
                    {
                        Text("Play Again",fontWeight= FontWeight.Bold, fontFamily= tictactoetitle,fontSize = 30.sp,
                            color= Color.Yellow)
                    }
                }
            }
